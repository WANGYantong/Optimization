function solution = Greedy_mod(Flows)
...
Wsize=data.W_k(1:NF);
probability=data.probability(1:NF,:);
Rspace=data.Zeta_e;
% Fullspace=data.W_e;
Rtotal=data.Zeta_t;
% utilization=data.utilization;
graph=data.graph;
delta=data.delta(1:NF);

punish=log(max(delta)+50-delta)*200;
...
%     else
%         total_cost=total_cost+punish(ii);
%     end
